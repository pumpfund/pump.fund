...
if __name__ == "__main__":
    ...
    # ==============================================================================
    # 🎯 LIVE PRODUCTION RUNTIME EXECUTION
    # ==============================================================================
    # Replace with the real Contract Address you are dropping to
    PRODUCTION_MINT = "HserEp6CMMETuArMjYT4LWMxiXz6i8p7BGVa8vZq7Ddk"
    ACTIVE_TICKER = "COAL"
    CURRENT_TOKEN_PRICE = 0.0005

    # Quantitative supply parameters for distribution calculation
    TOTAL_TOKENS_AVAILABLE_TO_AIRDROP = 1000000

    logger.info("📡 Fetching live holder weights from utility snapshot matrix...")
    try:
        raw_holder_data = holder_utils.get_holder_percentages()
        if not raw_holder_data:
            logger.error("❌ Aborting: Holder utility file returned an empty dataset.")
            sys.exit(1)

        # Map list of dicts [{"address": "...", "percentage": ...}] to clean Dict[str, float]
        live_snapshot_matrix = {}
        for item in raw_holder_data:
            if "address" in item and "percentage" in item:
                live_snapshot_matrix[item["address"]] = float(item["percentage"])

        logger.info(
            f"📊 Extracted {len(live_snapshot_matrix)} validated target recipients."
        )

    except Exception as e:
        logger.critical(f"❌ Failed to parse data from holder_utils engine: {e}")
        sys.exit(1)

    # Trigger distribution logic
    logger.info(f"🚀 INITIATING LIVE ON-CHAIN AIRDROP FOR ${ACTIVE_TICKER}...")
    start_time = time.time()

    try:
        AIRDROP(
            holder_percentages=live_snapshot_matrix,
            TOKEN_MINT=PRODUCTION_MINT,
        ...
        logger.info(f"🎉 SUCCESS: Airdrop pipeline finalized in {elapsed:.2f} seconds!")

    except Exception as run_err:
        logger.critical(f"💥 PIPELINE CRASHED: Execution aborted. Error: {run_err}")
